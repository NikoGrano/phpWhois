...
function generic_parser_a_blocks($rawdata, $translate, &$disclaimer)
{
    ...
    while (list($key, $val) = \each($rawdata)) {
        ...
        if ('' !== $val && ('%' === $val[0] || '#' === $val[0])) {
            if (!$dend) {
                $disclaimer[] = \trim(\mb_substr($val, 1));
            }
            continue;
        }
        if ('' === $val) {
    ...
    while (list($key, $val) = \each($rawdata)) {
        if ('' !== \trim($val)) {
            if (('%' === $val[0] || '#' === $val[0]) && $disok) {
                $r['disclaimer'][] = \trim(\mb_substr($val, 1));
                $disok = true;
                continue;
            }
            ...
            while (list($match, $field) = \each($items)) {
                $pos = \mb_strpos($val, $match);

                if (false !== $pos) {
                    if ('' !== $field) {
                        $var = '$r'.getvarname($field);
                        $itm = \trim(\mb_substr($val, $pos + \mb_strlen($match)));
...
function get_blocks($rawdata, $items, $partial_match = false, $def_block = false)
{
    ...
    while (list($key, $val) = \each($rawdata)) {
        ...
        while (list($key, $val) = \each($rawdata)) {
            ...
            if ('' === $val || $val === \str_repeat($val[0], \mb_strlen($val))) {
                continue;
            }
...
function get_contact($array, $extra_items = [], $has_org = false)
{
    ...
    while (list($key, $val) = \each($array)) {
        ...
        while ($ok) {
            ...
            while (list($match, $field) = \each($items)) {
                $pos = \mb_strpos(\mb_strtolower($val), $match);
            ...
            if (\preg_match("/([+]*[-\(\)\. x0-9]){7,}/", $val, $matches)) {
                ...
                if (\mb_strlen($phone) > 8 && !\preg_match('/[0-9]{5}\-[0-9]{3}/', $phone)) {
                    if (isset($r['phone'])) {
                        if (isset($r['fax'])) {
                            continue;
                        }
...
{
    $months = ['jan' => 1, 'ene' => 1, 'feb' => 2, 'mar' => 3, 'apr' => 4, 'abr' => 4,
        'may'        => 5, 'jun' => 6, 'jul' => 7, 'aug' => 8, 'ago' => 8, 'sep' => 9,
        'oct'        => 10, 'nov' => 11, 'dec' => 12, 'dic' => 12, ];

    $parts = \explode(' ', $date);

    if (false !== \mb_strpos($parts[0], '@')) {
        unset($parts[0]);
        $date = \implode(' ', $parts);
    }
    ...
    if ((8 === \mb_strlen($parts[0]) || 1 === \count($parts)) && \is_numeric($parts[0])) {
        $val = $parts[0];
        for ($p = $i = 0; $i < 3; ++$i) {
            if ('Y' !== $format[$i]) {
                $res[$format[$i]] = \mb_substr($val, $p, 2);
                $p += 2;
            } else {
                $res['y'] = \mb_substr($val, $p, 4);
                $p += 4;
            }
        }
    } else {
        ...
        for ($p = $i = 0; $p < \count($parts) && $i < \mb_strlen($format); ++$p) {
            if ('' === \trim($parts[$p])) {
                continue;
            }
    ...
    while (!$ok) {
        ...
        while (list($key, $val) = \each($res)) {
            ...
            if (!\is_numeric($val) && isset($months[\mb_substr(\mb_strtolower($val), 0, 3)])) {
                $res[$key] = $res['m'];
                $res['m'] = $months[\mb_substr(\mb_strtolower($val), 0, 3)];
                $ok = false;
                break;
            }
